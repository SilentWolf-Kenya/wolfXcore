...
Route::group(['prefix' => 'plans'], function () {
    ...
    Route::get('/view/{plan:id}', [Admin\PlanController::class, 'view'])->name('admin.plans.view');

    Route::post('/', [Admin\PlanController::class, 'store']);
    Route::patch('/view/{plan:id}', [Admin\PlanController::class, 'update']);
});

/*
|--------------------------------------------------------------------------
| Bot Repository Routes (Super Admin only — guarded in controller)
|--------------------------------------------------------------------------
*/
Route::group(['prefix' => 'bots'], function () {
    Route::get('/',                                     [Admin\BotRepoController::class, 'index'])->name('admin.bots');
    Route::get('/new',                                  [Admin\BotRepoController::class, 'view'])->name('admin.bots.new');
    Route::get('/view/{repo}',                          [Admin\BotRepoController::class, 'view'])->name('admin.bots.view');
    Route::post('/',                                    [Admin\BotRepoController::class, 'store'])->name('admin.bots.store');
    Route::post('/fetch-app-json',                      [Admin\BotRepoController::class, 'fetchAppJson'])->name('admin.bots.fetch');
    Route::post('/view/{repo}/prepare',                 [Admin\BotRepoController::class, 'prepareSlots'])->name('admin.bots.prepare');
    Route::patch('/view/{repo}',                        [Admin\BotRepoController::class, 'update'])->name('admin.bots.update');
    Route::delete('/view/{repo}/slots/{allocation}',    [Admin\BotRepoController::class, 'removeSlot'])->name('admin.bots.slot.remove');
});

// Super Admin routes are registered in RouteServiceProvider outside the admin middleware group.
