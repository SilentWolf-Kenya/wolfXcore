...
{
    public const VERSION_CACHE_KEY = 'wolfxcore:versioning_data';
    ...
    {
        return Arr::get(self::$result, 'discord') ?? 'https://wolfxcore.io/discord';
    }
    ...
    {
        return $this->cache->remember(self::VERSION_CACHE_KEY, CarbonImmutable::now()->addMinutes(config('wolfxcore.cdn.cache_time', 60)), function () {
            try {
                $response = $this->client->request('GET', config('wolfxcore.cdn.url'));
