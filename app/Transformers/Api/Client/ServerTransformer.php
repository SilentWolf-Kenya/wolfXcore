...
class ServerTransformer extends BaseClientTransformer
{
    ...
    public function transform(Server $server): array
    {
        ...
        return [
            ...
            'identifier' => config('wolfxcore.features.new_server_identifiers')
                ? $server->identifier
                : $server->uuidShort,
            '__deprecated_uuid_short' => $server->uuidShort,
            // In wolfXcore 2.0 we'll be replacing `identifier` above with the actual
            ...
            // wolfXcore breaker status (drives the React "needs attention" banner).
            'wxn_health' => $this->buildWxnHealth($server),
        ];
    }

    protected function buildWxnHealth(Server $server): array
    {
        $empty = [
            'paused' => false, 'needs_attention' => false, 'crash_count' => 0,
            'last_crash_at' => null, 'last_crash_reason' => null, 'paused_until' => null,
            'session_restores' => 0, 'last_session_restore_at' => null,
        ];
        try {
            $h = WxnBotHealth::where('server_id', $server->id)->first();
            if (!$h) return $empty;
            $paused = $h->isPaused();
            return [
                'paused'              => $paused,
                'needs_attention'     => $paused || (int) $h->crash_count_24h >= 2,
                'crash_count'         => (int) $h->crash_count_24h,
                'last_crash_at'       => $h->last_crash_at?->toIso8601String(),
                'last_crash_reason'   => $h->last_crash_reason,
                'paused_until'        => $paused ? $h->circuit_paused_until?->toIso8601String() : null,
                'session_restores'    => (int) ($h->session_restores ?? 0),
                'last_session_restore_at' => $h->last_session_restore_at?->toIso8601String(),
            ];
        } catch (\Throwable $e) {
            return $empty;
        }
