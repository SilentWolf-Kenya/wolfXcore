...
@section('footer-scripts')
    ...
    $(document).ready(function () {
        $('#pEggId').select2({placeholder: 'Select a Nest Egg'}).on('change', function () {
            ...
            var parentChain = _.get(wolfXcore.nests, $("#pNestId").val());
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (objectChain.id === parseInt(wolfXcore.server.egg_id) && wolfXcore.server.image == opt.value) {
                    opt.selected = true
                }
            ...
            if (objectChain.id === parseInt(wolfXcore.server.egg_id)) {
                if ($('#pDockerImage').val() != wolfXcore.server.image) {
                    $('#pDockerImageCustom').val(wolfXcore.server.image);
                }
            }
            ...
            $.each(_.get(objectChain, 'variables', []), function (i, item) {
                var setValue = _.get(wolfXcore.server_variables, item.env_variable, item.default_value);
        ...
        $('#pNestId').select2({placeholder: 'Select a Nest'}).on('change', function () {
            $('#pEggId').html('').select2({
                data: $.map(_.get(wolfXcore.nests, $(this).val() + '.eggs', []), function (item) {
                    return {
                        id: item.id,
                        text: item.name,
                    };
            ...
            if (_.isObject(_.get(wolfXcore.nests, $(this).val() + '.eggs.' + wolfXcore.server.egg_id))) {
                $('#pEggId').val(wolfXcore.server.egg_id);
            }
